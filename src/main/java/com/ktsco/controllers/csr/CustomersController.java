...
public class CustomersController implements Initializable {
	...
	private void populateCustomerTable() {
		...
		list = CustomerDAO.getAllCustomerRecord();
		generateTableColumns(list);
	}

	private void stablishingCustomerCode() {
		int code = CustomerDAO.stablishCustomerCode();
	...
	private void addCustomerRecord() {
		...
		if (code != 0 && !"".equalsIgnoreCase(company) && !"".equalsIgnoreCase(currency)) {
			boolean isExist = CustomerDAO.checkForDuplicateCustomer(company, currency);
			if (!isExist) {
				isSuccess = CustomerDAO.insertCustomerRecord(code, company, poc, phone, address, currency);
				clearTextFields();
			} else
	...
	private void deleteCustomerRecord() {
		if (!tableCustomerList.getSelectionModel().isEmpty()) {
			...
			if (AlertsUtils.askForDeleteAlert(company)) {
				isSuccess = CustomerDAO.deleteCustomerRecord(code);
			}
	...
	private void searchCustomerRecord() {
		...
		ObservableList<CustomerModel> list = CustomerDAO.searchCustomerRecord(company, poc, phone, address, currency);
		generateTableColumns(list);
	}
